...
import { Quote } from 'lucide-react'; // Importamos el icono de comillas

interface Props {
  data: CaseStudy['challengeSection'];
}

const CaseStudyChallenge: React.FC<Props> = ({ data }) => {
  return (
    <section className="bg-gray-50 py-24">
      <div className="container mx-auto px-4">
          ...
          {data.challenges.map((challenge, index) => (
            ...
            >
              ...
              <p className="font-semibold text-red-700">
                <span className="font-bold">Impacto:</span> {challenge.impact}
              </p>
            </motion.div>
          ))}
        </div>

        {/* --- SECCIÓN "THE CORE PROBLEM" REDISEÑADA --- */}
        <div className="mt-20 max-w-4xl mx-auto">
          ...
          >
            {/* Icono de comillas gigante como fondo decorativo */}
            <Quote className="absolute top-4 left-4 h-24 w-24 text-gray-100/80 -translate-x-4 -translate-y-4" />
            
            <div className="relative text-center z-10">
              <h3 className="text-2xl font-bold text-gray-900 mb-6">{data.coreProblem.title}</h3>
              <blockquote className="text-xl md:text-2xl italic text-slate-700 leading-relaxed">
                "{data.coreProblem.quote}"
              </blockquote>
              <cite className="block mt-8 text-slate-500 font-semibold not-italic">— {data.coreProblem.author}</cite>
            </div>
          </motion.div>
        </div>
