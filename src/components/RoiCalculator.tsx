...
// Datos simplificados de los planes para los cálculos.
// Los valores de 'savedHoursPerRep' y 'leadIncrease' son estimaciones que deberás ajustar a tu modelo de negocio.
const plans = [
...
const RoiCalculator = () => {
  ...
  // Formateador de moneda para mostrar los números correctamente
  const formatCurrency = (value: number) => new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 0 }).format(value);

  // useMemo recalcula los resultados solo cuando una de las dependencias cambia, optimizando el rendimiento.
  const calculation = useMemo(() => {
    ...
    // Asumimos una tasa de cierre del 10% para los nuevos leads generados por la IA para este cálculo
    const additionalDeals = additionalLeads * 0.10; 
    const revenueGain = additionalDeals * avgDealSize;
    
    // Asumimos un coste/valor de $50/hora por empleado para calcular el ahorro
  ...
  return (
    <section id="roi-calculator" className="py-20 md:py-24 bg-[#F1F3F5]">
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        ...
        >
          <div className="bg-white p-6 md:p-10 rounded-2xl shadow-xl border border-slate-200">
            <h2 className="text-3xl md:text-4xl font-bold text-center mb-2 text-[#0D1B2A]">Calculate Your Potential ROI</h2>
            <p className="text-center text-gray-500 mb-8 md:mb-12">Adjust the sliders to see your estimated savings and revenue growth.</p>
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-2 mb-8 bg-slate-100 p-1.5 rounded-full max-w-lg mx-auto">
              {plans.map(plan => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full font-semibold transition-all w-full text-sm
                    ${selectedPlanId === plan.id ? 'bg-[#1C7ED6] text-white shadow' : 'text-gray-600 hover:bg-slate-200'}
                  `}
                >
                  {plan.name}
                </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
              ...
              <div className="bg-slate-50 p-6 rounded-lg text-center flex flex-col justify-center border border-slate-200">
                  ...
                  <motion.p 
                    ...
                    className="text-4xl md:text-5xl font-bold text-[#69DB7C] my-2" // Tu verde de acento
                  >
